...
- **Editor** CodeMirror 6: highlighting multi-linguaggio (lazy), salvataggio, auto-reload
  dei file modificati dall'esterno (es. da Claude) e **find/replace** (Ctrl+F).
- **Quick-open** dei file per nome con ranking fuzzy (Ctrl+P).
- **Ricerca** testuale in tutto il progetto.
- **Git** locale: status, diff, stage/unstage, commit, cambio ramo, annulla modifiche
  (discard) e cronologia dei commit con diff per commit.
- **Terminale** integrato (PTY reale) docked a destra, con **tab multiple** e scelta della
  shell, + finestra flottante always-on-top — tieni `claude` in una tab e i comandi in un'altra.
